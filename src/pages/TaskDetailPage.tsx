...
const taskSchema = z.object({
  ...
  area_id: z.coerce.number().optional(), assigned_team_id: z.coerce.number().optional(), due_at: z.string().optional(), payload_json: z.string().optional(),
})
...
export function TaskDetailPage() {
  ...
  useEffect(() => { if (!taskQuery.data) return; form.reset({ title: taskQuery.data.title, description: String(taskQuery.data.description ?? ''), status: taskQuery.data.status, priority: taskQuery.data.priority, latitude: taskQuery.data.latitude ?? undefined, longitude: taskQuery.data.longitude ?? undefined, area_id: taskQuery.data.area?.id, assigned_team_id: taskQuery.data.assigned_team?.id, due_at: taskQuery.data.due_at ? String(taskQuery.data.due_at).slice(0, 16) : '', payload_json: taskQuery.data.payload ? JSON.stringify(taskQuery.data.payload, null, 2) : '' }) }, [taskQuery.data?.id])
  ...
  return <section className="space-y-4"><div className="flex items-center justify-between"><h1 className="text-2xl font-semibold">Task #{task.id}: {task.title}</h1><Link className="text-blue-600" to={`/campaigns/${task.campaign_id}`}>Back to campaign</Link></div>
    <div className="rounded border bg-white p-4"><h2 className="mb-3 font-medium">Task overview</h2><p>Status: {task.status} · Priority: {task.priority}</p></div>
    <div className="rounded border bg-white p-4"><h2 className="mb-2 font-medium">Edit task form</h2><form className="space-y-2" onSubmit={form.handleSubmit((values) => updateMutation.mutate(values))}><input {...form.register('title')}  disabled={editDisabled} title={editDisabled ? NO_PERMISSION_MESSAGE : undefined} /><textarea rows={3} {...form.register('description')}  disabled={editDisabled} title={editDisabled ? NO_PERMISSION_MESSAGE : undefined} />
      ...
      <div className="grid grid-cols-2 gap-2"><select {...form.register('area_id')} disabled={editDisabled} title={editDisabled ? NO_PERMISSION_MESSAGE : undefined}><option value="">Area</option>{(areasQuery.data?.data ?? []).map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}</select><select {...form.register('assigned_team_id')} disabled={editDisabled} title={editDisabled ? NO_PERMISSION_MESSAGE : undefined}><option value="">Assigned team</option>{(teamsQuery.data?.data ?? []).map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}</select></div>
    ...
    </form>{(updateMutation.isError || deleteMutation.isError) && <ErrorState message={permissionErrorMessage(updateMutation.error ?? deleteMutation.error)} />}</div>
    <div className="rounded border bg-white p-4"><h2 className="font-medium">Events</h2>{eventsQuery.data?.data.map((event) => <div key={event.id}><p>{event.type}</p></div>)}<div className="mt-2 flex gap-2"><button type="button" className="border px-2" onClick={() => setEventsPage((p) => Math.max(1, p - 1))}>Previous</button><button type="button" className="border px-2" onClick={() => setEventsPage((p) => p + 1)}>Next</button></div></div>
  </section>
}
