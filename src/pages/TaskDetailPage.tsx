...
export function TaskDetailPage() {
  ...
  const createPointMutation = useMutation({
    mutationFn: (payload: Partial<TaskPoint>) => createTaskPoint(id, payload),
    onSuccess: () => {
      invalidateAll()
      pointForm.reset({ label: '', description: '', latitude: DEFAULT_CENTER[0], longitude: DEFAULT_CENTER[1], sort_order: 0, payload_json: '' })
  ...
  return <section className="space-y-4">
    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
      <h1 className="text-2xl font-semibold">Auftrag #{task.id}: {task.title}</h1>
      <div className="flex flex-wrap gap-2">
        <Link className="border px-3 py-2 text-sm" to={`/campaigns/${task.campaign_id}`}>Zur Kampagne</Link>
        {isEditMode
          ? <Link className="border px-3 py-2 text-sm" to={`/tasks/${task.id}`}>Detailansicht</Link>
          : <Link className={`border px-3 py-2 text-sm ${!can(task.can?.update) ? 'pointer-events-none opacity-50' : ''}`} title={!can(task.can?.update) ? NO_PERMISSION_MESSAGE : undefined} to={`/tasks/${task.id}/edit`}>Auftrag bearbeiten</Link>}
      </div>
    </div>
    ...
    <div className="rounded border bg-white p-4">
      <details>
        <summary className="cursor-pointer font-medium">Punkte / Marker ({points.length})</summary>
        <div className="mt-3">
          {points.length === 0 ? <EmptyState message="Noch keine Punkte für diesen Auftrag vorhanden." /> : (
            <div className="space-y-2">
              {points.map((point) => (
                <article key={point.id} className="rounded border p-2 text-sm">
                  <p className="font-medium">{point.label ?? `Punkt #${point.id}`}</p>
                  <p>{point.description ?? '—'}</p>
                  <p>Koordinaten: {point.latitude}, {point.longitude}</p>
                  <p>Sortierung: {point.sort_order ?? 0}</p>
                  <div className="mt-2 flex gap-2">
                    <button
                      type="button"
                      className="border px-2 disabled:opacity-50"
                      disabled={!canManagePoints || !can(point.can?.update)}
                      title={!canManagePoints ? NO_PERMISSION_MESSAGE : undefined}
                      onClick={() => {
                        pointForm.reset({ id: point.id, label: point.label ?? '', description: point.description ?? '', latitude: point.latitude, longitude: point.longitude, sort_order: point.sort_order ?? 0, payload_json: point.payload ? JSON.stringify(point.payload, null, 2) : '' })
                        setPointEditorOpen(true)
                      }}
                    >
                      Punkt bearbeiten
                    </button>
                    <button type="button" className="border px-2 disabled:opacity-50" disabled={!canManagePoints || !can(point.can?.delete)} title={!canManagePoints ? NO_PERMISSION_MESSAGE : undefined} onClick={() => deletePointMutation.mutate(point.id)}>
                      Punkt entfernen
                    </button>
                  </div>
                </article>
    ...
    <div className="rounded border bg-white p-4">
      <details open={pointEditorOpen || isEditMode} onToggle={(event) => setPointEditorOpen(event.currentTarget.open)}>
        <summary className="cursor-pointer font-medium">Punkt hinzufügen / Punkt bearbeiten</summary>
        <div className="mt-3">
          <form className="space-y-2" onSubmit={pointForm.handleSubmit(savePoint)}>
            <input placeholder="Label" {...pointForm.register('label')} disabled={!canManagePoints} title={!canManagePoints ? NO_PERMISSION_MESSAGE : undefined} />
            <textarea rows={2} placeholder="Beschreibung" {...pointForm.register('description')} disabled={!canManagePoints} title={!canManagePoints ? NO_PERMISSION_MESSAGE : undefined} />
            <div className="grid gap-2 md:grid-cols-2">
              <input type="number" step="any" placeholder="Breitengrad" {...pointForm.register('latitude')} disabled={!canManagePoints} />
              <input type="number" step="any" placeholder="Längengrad" {...pointForm.register('longitude')} disabled={!canManagePoints} />
            </div>
            <input type="number" placeholder="Sortierung" {...pointForm.register('sort_order')} disabled={!canManagePoints} />
            <textarea rows={4} placeholder="Payload JSON" {...pointForm.register('payload_json')} disabled={!canManagePoints} />
            <button type="submit" className="bg-slate-900 px-3 py-1 text-white disabled:opacity-50" disabled={!canManagePoints} title={!canManagePoints ? NO_PERMISSION_MESSAGE : undefined}>{pointForm.watch('id') ? 'Punkt bearbeiten' : 'Punkt hinzufügen'}</button>
          </form>
          {pointForm.formState.errors.latitude && <ErrorState message={pointForm.formState.errors.latitude.message ?? ''} />}
          {pointForm.formState.errors.longitude && <ErrorState message={pointForm.formState.errors.longitude.message ?? ''} />}
          {pointFormError && <ErrorState message={pointFormError} />}
          {pointOutsideHint && <p className="text-xs text-amber-700">Der Punkt liegt möglicherweise außerhalb des Zielgebiets.</p>}
        </div>
      </details>
    </div>
    ...
    <div className="rounded border bg-white p-4">
      <details>
        ...
        <div className="mt-3 h-96 overflow-hidden rounded border">
          {mapAreas.length === 0 && points.length === 0 ? <div className="p-3 text-sm text-slate-600">Keine Kartenobjekte für diesen Auftrag vorhanden.</div> : (
            <MapContainer center={DEFAULT_CENTER} zoom={6} className="h-full w-full">
              <TileLayer attribution={MAP_ATTRIBUTION} url={MAP_TILE_URL} />
              <MapClickPicker onPick={(lat, lng) => { pointForm.setValue('latitude', Number(lat.toFixed(6))); pointForm.setValue('longitude', Number(lng.toFixed(6))) }} />
              {mapAreas.map((area, index) => <GeoJSON key={area.id} data={area.geojson as GeoJSON.GeoJsonObject} style={{ color: index === 0 ? '#1d4ed8' : '#0f766e', weight: index === 0 ? 4 : 2, fillOpacity: index === 0 ? 0 : 0.2 }} />)}
              {points.map((point) => (
                <Marker
                  key={point.id}
                  position={[point.latitude, point.longitude]}
                  draggable={canManagePoints && can(point.can?.update)}
                  eventHandlers={{ dragend: (event) => { const latLng = event.target.getLatLng(); updatePointMutation.mutate({ id: point.id, data: { latitude: latLng.lat, longitude: latLng.lng } }) } }}
                >
                  <Popup><p className="font-medium">{point.label ?? `Punkt #${point.id}`}</p><p>{point.description ?? '—'}</p><p>Koordinaten: {point.latitude}, {point.longitude}</p></Popup>
                </Marker>
              ))}
              <FitMap areas={mapAreas} points={points} />
            </MapContainer>
          )}
        </div>
      </details>
    ...
    {isEditMode && (
      <div className="rounded border bg-white p-4">
        ...
        <form className="space-y-2" onSubmit={taskForm.handleSubmit((values) => updateTaskMutation.mutate(values))}>
          <input {...taskForm.register('title')} disabled={!can(task.can?.update)} title={!can(task.can?.update) ? NO_PERMISSION_MESSAGE : undefined} />
          <textarea rows={3} {...taskForm.register('description')} disabled={!can(task.can?.update)} title={!can(task.can?.update) ? NO_PERMISSION_MESSAGE : undefined} />
          <label className="block text-sm">Briefing</label>
          <textarea rows={4} {...taskForm.register('briefing')} disabled={!can(task.can?.update)} title={!can(task.can?.update) ? NO_PERMISSION_MESSAGE : undefined} />
          <p className="text-xs text-slate-600">Konkrete Arbeitsanweisungen für diesen Auftrag.</p>
          <div className="grid gap-2 md:grid-cols-2">
            <select {...taskForm.register('status')} disabled={!can(task.can?.change_status)} title={!can(task.can?.change_status) ? NO_PERMISSION_MESSAGE : undefined}>{TASK_STATUSES.map((status) => <option key={status}>{status}</option>)}</select>
            <input type="number" min={1} max={5} {...taskForm.register('priority')} disabled={!can(task.can?.update)} title={!can(task.can?.update) ? NO_PERMISSION_MESSAGE : undefined} />
          </div>
          <div className="grid gap-2 md:grid-cols-3">
            <select {...taskForm.register('boundary_area_id')} disabled={!can(task.can?.update)}><option value="">Begrenzung auswählen</option>{campaignAreas.filter((a) => a.pivot?.usage === 'boundary').map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}</select>
            <select {...taskForm.register('area_id')} disabled={!can(task.can?.update)}><option value="">Zielgebiet auswählen</option>{campaignAreas.filter((a) => a.pivot?.usage === 'target').map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}</select>
            <select {...taskForm.register('assigned_team_id')} disabled={!can(task.can?.update) || !can(task.can?.assign_team)} title={!can(task.can?.assign_team) ? NO_PERMISSION_MESSAGE : undefined}><option value="">Zugewiesenes Team</option>{(teamsQuery.data?.data ?? []).map((team) => <option key={team.id} value={team.id}>{team.name}</option>)}</select>
          </div>
          <input type="datetime-local" {...taskForm.register('due_at')} disabled={!can(task.can?.update)} title={!can(task.can?.update) ? NO_PERMISSION_MESSAGE : undefined} />
          <textarea rows={6} placeholder="Payload JSON" {...taskForm.register('payload_json')} disabled={!can(task.can?.update)} title={!can(task.can?.update) ? NO_PERMISSION_MESSAGE : undefined} />
          <div className="flex gap-2">
            <button type="submit" className="bg-slate-900 text-white disabled:opacity-50" disabled={!can(task.can?.update)} title={!can(task.can?.update) ? NO_PERMISSION_MESSAGE : undefined}>Speichern</button>
            <button type="button" className="bg-red-600 text-white disabled:opacity-50" disabled={!can(task.can?.delete)} title={!can(task.can?.delete) ? NO_PERMISSION_MESSAGE : undefined} onClick={() => deleteTaskMutation.mutate()}>Löschen</button>
          </div>
        </form>
        {updateTaskMutation.isError && <ErrorState message={requestErrorMessage(updateTaskMutation.error)} />}
        {deleteTaskMutation.isError && <ErrorState message={requestErrorMessage(deleteTaskMutation.error)} />}
      </div>
    )}

    <div className="rounded border bg-white p-4">
      <details>
        <summary className="cursor-pointer font-medium">Ereignisse</summary>
        <div className="mt-3">
          {eventsQuery.data?.data.map((event) => <div key={event.id}><p>{event.event_type}</p></div>)}
          <div className="mt-2 flex gap-2">
            <button type="button" className="border px-2" onClick={() => setEventsPage((page) => Math.max(1, page - 1))}>Zurück</button>
            <button type="button" className="border px-2" onClick={() => setEventsPage((page) => page + 1)}>Weiter</button>
          </div>
        </div>
      </details>
    </div>
