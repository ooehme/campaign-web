...
const requestErrorMessage = (error: unknown) => {
  ...
  if (error.status === 404) return 'Auftrag oder Punkt nicht gefunden.'
...
export function TaskDetailPage() {
  ...
  const taskForm = useForm<TaskFormValues>({ resolver: zodResolver(taskSchema), defaultValues: { title: '', description: '', status: 'open', priority: 3, payload_json: '' } })
  ...
  if (!Number.isFinite(id)) return <ErrorState message="Auftrag oder Punkt nicht gefunden." />
  ...
  if (!taskQuery.data) return <EmptyState message="Auftrag oder Punkt nicht gefunden." />

  const task = taskQuery.data
  const points = (pointsQuery.data ?? task.points ?? []).slice().sort((a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0))
  ...
  const canManagePoints = can(task.can?.manage_points)
