...
export function UserDetailPage() {
  ...
  return <section className="space-y-4">
    ...
    <div className="rounded border bg-white p-4">
      ...
      <p>ID: {user.id}</p>
      <p>Erstellt: {user.created_at ?? 'nicht verfügbar'}</p>
      <p>Aktualisiert: {user.updated_at ?? 'nicht verfügbar'}</p>
    </div>

    <div className="rounded border bg-white p-4">
      <details>
        <summary className="cursor-pointer font-medium">Teams ({teams.length})</summary>
        <div className="mt-3 overflow-auto">{teams.length === 0 ? <EmptyState message="Dieser Benutzer ist noch keinem Team zugewiesen." /> : <table className="min-w-[640px] w-full text-sm"><thead><tr className="text-left"><th>Team</th><th>Rolle</th><th>Anzeigename</th><th>Notizen</th></tr></thead><tbody>{teams.map((t) => <tr key={t.id} className="border-t"><td><Link className="text-blue-600" to={`/teams/${t.id}`}>{t.name}</Link></td><td>{roleLabels[t.pivot?.role ?? 'member']}</td><td>{t.pivot?.display_name ?? '-'}</td><td>{t.pivot?.notes ?? '-'}</td></tr>)}</tbody></table>}</div>
      </details>
    </div>

    <div className="rounded border bg-white p-4">
      <details>
        <summary className="cursor-pointer font-medium">Kampagnen ({campaigns.length})</summary>
        <div className="mt-3">{campaigns.length === 0 ? <EmptyState message="Keine zugehörigen Kampagnen gefunden." /> : <ul>{campaigns.map((c) => <li key={c.id}><Link className="text-blue-600" to={`/campaigns/${c.id}`}>{c.name}</Link> ({c.status ?? '-'})</li>)}</ul>}</div>
      </details>
    </div>

    <div className="rounded border bg-white p-4"><h2 className="font-medium">Auftrags-Zusammenfassung</h2><ul><li>offen: {taskSummary.open}</li><li>zugewiesen: {taskSummary.assigned}</li><li>in Bearbeitung: {taskSummary.in_progress}</li><li>erledigt: {taskSummary.done}</li><li>abgebrochen: {taskSummary.cancelled}</li><li>gesamt: {taskSummary.total}</li></ul></div>

    <div className="rounded border bg-white p-4 space-y-2">
      <details>
        <summary className="cursor-pointer font-medium">Aufträge</summary>
        <div className="mt-3 space-y-2 overflow-auto"><div className="flex gap-2 flex-wrap">{taskFilters.map((f) => <button key={f.key} className={`border px-2 py-1 text-xs ${taskFilter === f.key ? 'bg-slate-900 text-white' : ''}`} onClick={() => setTaskFilter(f.key)}>{f.label}</button>)}</div>{tasksQuery.isLoading && <LoadingState />}{tasksQuery.isError && <p className="text-sm text-slate-600">Auftrags-Endpunkt derzeit nicht verfügbar.</p>}{!tasksQuery.isLoading && !tasksQuery.isError && tasks.length === 0 && <EmptyState message="Keine Aufträge gefunden." />}{tasks.length > 0 && <table className="min-w-[640px] w-full text-sm"><thead><tr className="text-left"><th>Titel</th><th>Status</th><th>Priorität</th><th>Kampagne</th><th>Team</th><th>Fällig</th></tr></thead><tbody>{tasks.map((task) => <tr className="border-t" key={task.id}><td><Link className="text-blue-600" to={`/tasks/${task.id}`}>{task.title}</Link></td><td>{task.status}</td><td>{task.priority}</td><td>{task.campaign_id}</td><td>{task.assigned_team?.name ?? '-'}</td><td>{task.due_at ?? '-'}</td></tr>)}</tbody></table>}</div>
      </details>
    </div>
    ...
    <div className="rounded border bg-white p-4">
      <details>
        ...
        <div className="mt-3 overflow-auto">{!canViewInvitations && <p className="text-sm text-slate-600">Einladungen sind nur im eigenen Profil verfügbar.</p>}{canViewInvitations && invitationsQuery.isError && <p className="text-sm text-slate-600">Einladungen-Endpunkt derzeit nicht verfügbar.</p>}{canViewInvitations && invitations.length === 0 ? <EmptyState message="Keine offenen Einladungen." /> : canViewInvitations && <table className="min-w-[640px] w-full text-sm"><thead><tr className="text-left"><th>Team</th><th>Rolle</th><th>Eingeladen von</th><th>Läuft ab</th><th>Notizen</th></tr></thead><tbody>{invitations.map((inv) => <tr key={inv.id} className="border-t"><td>{inv.team?.name ?? '-'}</td><td>{roleLabels[inv.role]}</td><td>{inv.invited_by_user?.name ?? '-'}</td><td>{inv.expires_at ?? '-'}</td><td>{inv.notes ?? '-'}</td></tr>)}</tbody></table>}</div>
      </details>
    </div>
  </section>
}
